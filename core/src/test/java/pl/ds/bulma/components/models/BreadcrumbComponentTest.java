...
class BreadcrumbComponentTest {
    ...
    public void init() {
        ...
        context.load().json(requireNonNull(
                Thread.currentThread().getContextClassLoader().getResourceAsStream("breadcrumb.json")), PATH);
    }

    @Test
    void defaultPricingPlanComponentModelTest() {
        BreadcrumbComponent model = context.resourceResolver().getResource(PATH + "/default").adaptTo(BreadcrumbComponent.class);
    ...
    void pricingPlanComponentModelTest() {
        BreadcrumbComponent model = context.resourceResolver().getResource(PATH + "/complex").adaptTo(BreadcrumbComponent.class);
        ...
        assertThat(model.getClasses()).containsExactlyInAnyOrder("center", "is-small", "has-bullet-separator");
